        ...
        <header>
            ...
            <div class="mt-3">
                 ...
                 <div v-for="(item, index) in formpekerjaan.deskripsi && formpekerjaan.id" :key="index" class="mt-3 overflow-hidden">
                   <table  class="min-w-full text-left text-sm font-light text-surface dark:text-black">
                    ...
                    <tr>
                        ...
                        <th scope="col" class="px-1 py-1" v-if="!formpekerjaan.status[index]">Checklist</th>
                ...
                <td class="whitespace-nowrap font-medium" v-if="users == 'admin'">
                    <PrimaryButton class="ml-2" @click="openModalEdit(formpekerjaan.id[index],formpekerjaan.deskripsi[index])"  :class="{'opacity-65' : formpekerjaan.status[index] == true}" :disabled="formpekerjaan.status[index] == true" v-if="!formpekerjaan.status[index]"><i class="far fa-edit"></i> Edit</PrimaryButton>
                    <DangerButton class="ml-2" @click="openModalDelete(formpekerjaan.id[index])" :class="{'opacity-65' : formpekerjaan.status[index] == true}" :disabled="formpekerjaan.status[index] == true"  v-if="!formpekerjaan.status[index]"><i class="fa-solid fa-trash"></i> Hapus</DangerButton>
                </td>
                <td class="whitespace-nowrap px-6 py-4 font-medium" v-if="users == 'pemilik_lahan'">
                <Checkbox @click="check(index)" v-model="formpekerjaan.status[index]" class="bg-gray-400 opacity-50" :checked="formpekerjaan.status[index] == true" v-if="!formpekerjaan.status[index]" disabled/>
                </td>
                ...
                <Checkbox :class="{'opacity-50' : formpekerjaan.status[index] == true}" :disabled="formpekerjaan.status[index] == true" @click="check(index)" v-model="formpekerjaan.status[index]" class="bg-gray-400" :checked="formpekerjaan.status[index] == true" v-if="!formpekerjaan.status[index]"/>
                </td>
                <td class="whitespace-nowrap px-6 py-4" v-if="users == 'petani'">
